...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_voyager(
    ...
    DK_PLUS,        CKC_A,          CKC_S,          CKC_D,          CKC_F,          KC_G,                                           KC_H,           CKC_J,          CKC_K,          CKC_L,          CKC_AE,         DK_OSTR,        
...
void on_smtd_action(uint16_t keycode, smtd_action action, uint8_t tap_count) {
    switch (keycode) {
        SMTD_MTE(CKC_A, KC_A, KC_LGUI)
        SMTD_MTE(CKC_S, KC_S, KC_LALT)
        SMTD_MTE(CKC_D, KC_D, KC_LCTL)
        SMTD_MTE(CKC_F, KC_F, KC_LSFT)
        SMTD_MTE(CKC_J, KC_J, KC_RSFT)
        SMTD_MTE(CKC_K, KC_K, KC_RCTL)
        SMTD_MTE(CKC_L, KC_L, KC_RALT)
        SMTD_MTE(CKC_AE, DK_AE, KC_RGUI)
    }
}
#endif
